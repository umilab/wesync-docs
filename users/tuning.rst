...
Wesync ships with a set of default values designed to work well for most
...
- :opt:`setLowPriority`
    Set to false to run Wesync with normal priority. This may allow
    Wesync to use more CPU time, which can improve performance.
...
- :opt:`weakHashThresholdPct`
    Wesync will by default look for rolling (weak) hash matches to detect
...
- :opt:`disableFsync`
    Wesync calls ``fsync()`` on files and directories after syncing them
...
- :opt:`blockPullOrder`
    Wesync by default uses a pseudo-random block order when pulling in
...
- ``GOMEMLIMIT`` and ``GOGC``: These environment variables can be used to
  ...
  the desired max amount of memory Wesync should use can improve
...
You have limited resources and want Wesync to use as few as possible. You
...
- :opt:`copyRangeMethod`
    ...
    efficient than having Wesync do the data copying.
...
- ``GOMEMLIMIT`` and ``GOGC``
    ...
    amount of memory Wesync should use can make the garbage collector
...
- ``GOMAXPROCS``
    ...
    number concurrently running threads Wesync uses. Setting it to 1 (or
    any number lower than your actual number of cores) will reduce the
    amount of CPU used by Wesync at any given moment.
