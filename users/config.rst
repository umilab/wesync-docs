...
::
    ...
    $HOME/.config/wesync
    $HOME/Library/Application Support/Wesync
    %LOCALAPPDATA%\Wesync
...
Wesync uses a single directory to store configuration and crypto keys.
Wesync also has a database, which is often stored in this directory too.
The config location defaults to ``$HOME/.config/wesync``
(Unix-like), ``$HOME/Library/Application Support/Wesync`` (Mac),
or ``%LOCALAPPDATA%\Wesync`` (Windows). It can be changed at runtime
...
* If a database exists in the old default location, that location is
  still used.
* If ``$XDG_DATA_HOME`` is set, use ``$XDG_DATA_HOME/wesync``.
* If ``~/.local/share/wesync`` exists, use that location.
...
.. code-block:: xml
    ...
    <configuration version="37">
        ...
        <options>
            ...
            <urURL>https://data.wesync.net/newdata</urURL>
            ...
            <releasesURL>https://upgrades.wesync.net/meta.json</releasesURL>
            ...
            <crashReportingURL>https://crash.wesync.net/newcrash</crashReportingURL>
...
.. option:: folder.type
    ...
    Controls how the folder is handled by Wesync. Possible values are:
...
.. option:: folder.device
    ...
    Wesync will currently add this automatically if it is not present in
...
.. option:: folder.scanProgressIntervalS
    ...
    will cause Wesync to use the default value of two.
...
.. option:: folder.copyOwnershipFromParent
    ...
    Requires running Wesync as a privileged user, or granting it additional capabilities (e.g. CAP_CHOWN on Linux).
...
``device`` element for the local device; Wesync will currently add one if
...
.. option:: device.certName
    ...
    The device certificate's common name, if it is not the default "wesync".
...
.. option:: device.autoAcceptFolders
    ...
    folder name, Wesync tries to use the label from the remote device, and if
...
.. option:: gui.address
    ...
    Wildcard and port (``0.0.0.0:12345``, ``[::]:12345``, ``:12345``)
        These are equivalent and will result in Wesync listening on all
...
.. option:: gui.insecureAllowFrameLoading
    ...
    needed for serving the Wesync GUI as part of a website through a proxy.
...
.. code-block:: xml
    ...
    <ldap>
        ...
        <bindDN>cn=%s,ou=users,dc=wesync,dc=net</bindDN>
...
.. option:: options.startBrowser
    ...
    Whether to attempt to start a browser to show the GUI when Wesync starts.
...
.. option:: options.urAccepted
    ...
    ``0``, mean the user has not made a choice, and Wesync will ask at some
...
.. option:: options.setLowPriority
    ...
    Wesync will attempt to lower its process priority at startup.
    ...
    as part of launching Wesync, set this option to ``false``.
...
.. option:: options.crashReportingEnabled
    ...
    feature. Set ``false`` to keep Wesync from sending panic logs on serious
    troubles.  Defaults to ``true``, to help the developers troubleshoot.

.. option:: options.databaseTuning

    Controls how Wesync uses the backend key-value database that stores the
...
.. option:: options.sendFullIndexOnUpgrade
    ...
    equivalent to starting Wesync with :option:`--reset-deltas`.  This used
...
.. option:: options.insecureAllowOldTLSVersions
    ...
    Only for compatibility with old versions of Wesync on remote devices, as
...
If you want Wesync to listen on multiple addresses, you can either: add
...
Default listen addresses (``default``)
    ...
    and ``dynamic+https://relays.wesync.net/endpoint``.
...
TCP IPv4 address and port (``tcp4://192.0.2.1:22000``)
    This results in Wesync listening on the specified address and port, IPv4
    only.
...
TCP IPv6 address and port (``tcp6://[2001:db8::42]:22000``)
    This results in Wesync listening on the specified address and port, IPv6
    only.
...
Static relay address (``relay://192.0.2.42:22067?id=abcd123...``)
    Wesync will connect to and listen for incoming connections via the
    specified relay address.

    .. todo:: Document available URL parameters.

Dynamic relay pool (``dynamic+https://192.0.2.42/relays``)
    Wesync will fetch the specified HTTPS URL, parse it for a JSON payload
...
If you want to use Wesync to backup your configuration files, it is recommended
