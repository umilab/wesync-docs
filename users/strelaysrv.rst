...
Wesync relies on a network of community-contributed relay servers. Anyone
can run a relay server, and it will automatically join the relay pool and be
available to Wesync users. The current list of relays can be found at
https://relays.wesync.net/.
...
.. cmdoption:: -pools=<pool addresses>
    ...
    "https://relays.wesync.net/endpoint"). Blank to disable announcement to
...
Go to `releases <https://github.com/umilab/relaysrv/releases>`__ and
...
manager. Recent releases can be found at wesync's
`apt repository <https://apt.wesync.net/>`_. The name of the package is
wesync-relaysrv.
...
for providing public statistics at https://relays.wesync.net/.  The firewall, such as
...
Then configure your Wesync devices to send the token when joining the relay::
...
:manpage:`wesync-relay(7)`, :manpage:`wesync-faq(7)`,
:manpage:`wesync-networking(7)`
