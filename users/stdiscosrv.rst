...
Wesync relies on a discovery server to find peers on the internet. Anyone
can run a discovery server and point Wesync installations to it. The
Wesync project also maintains a global cluster for public use.
...
Pointing Wesync at Your Discovery Server
-------------------------------------------

By default, Wesync uses a number of global discovery servers, signified by
the entry ``default`` in the list of discovery servers. To make Wesync use
your own instance of stdiscosrv, open up Wesync's web GUI. Go to settings,
...
This guide assumes that you have already set up Wesync. If you
haven't yet, head over to :ref:`getting-started` first.

Installing
~~~~~~~~~~

Go to `releases <https://github.com/umilab/discosrv/releases>`__ and
...
.. note::
   If you are running an instance of Wesync on the discovery server,
   you must either add that instance to other devices using a static
   address or bind the discovery server and Wesync instances to
...
- Use any certificate pair and let clients authenticate the server based on
  its "device ID" (similar to Wesync-to-Wesync authentication). This
...
Authentication of replication connections is done using `Wesync-style
device IDs <https://docs.wesync.net/dev/device-ids.html#id1>`__ only - CA
...
Wesync towards this name. The same certificate must be used on both
discovery servers.
...
- The "X-SSL-Cert" HTTP header must be passed through with the PEM-encoded
  ...
  in GET requests from clients. If you see wesync-discosrv outputting
...
Server and Wesync using Nginx, `Let's Encrypt`_ and Docker can be found here_.

.. _Let's Encrypt: https://letsencrypt.org/
.. _here: https://forum.wesync.net/t/docker-wesync-and-wesync-discovery-behind-nginx-reverse-proxy-with-lets-encrypt/6880
...
`Reverse Proxy Setup <https://docs.wesync.net/users/reverseproxy.html>`__
page. Note that that page is directed at setting up a proxy for the
Wesync web UI. You should do the proper path and port adjustments to proxying
...
:manpage:`wesync-networking(7)`, :manpage:`wesync-faq(7)`
