...
::
    ...
    wesync decrypt (--to=<dir> | --verify-only)
...
Wesync lets you synchronize your files bidirectionally across multiple
...
The ``wesync`` core application is a command-line program which usually runs
...
.. cmdoption:: --browser-only
   ...
   Open the web UI in a browser for an already running Wesync instance.
...
.. cmdoption:: --gui-address=<address>
    ...
    ``unix:///run/wesync/wesync.socket`` (notice the three slashes: two
...
.. cmdoption:: --home=<dir>
    ...
    directory is ``$HOME/.config/wesync`` (Unix-like),
    ``$HOME/Library/Application Support/Wesync`` (Mac) and
    ``%LOCALAPPDATA%\Wesync`` (Windows).
...
.. cmdoption:: --no-restart
    ...
    Do not restart Wesync when it exits. The monitor process will still run
...
details.  It does not depend on Wesync to be running, but works on offline
data.

To work with the REST API for debugging or automating things in Wesync, the
...
Wesync can use a SOCKS, HTTP, or HTTPS proxy to talk to the outside
...
The following environment variables modify Wesync's behavior in ways that
are mostly useful for developers. Use with care.
If you start Wesync from within service managers like systemd or supervisor,
...
:manpage:`wesync-config(5)`, :manpage:`wesync-stignore(5)`,
:manpage:`wesync-device-ids(7)`, :manpage:`wesync-security(7)`,
:manpage:`wesync-networking(7)`, :manpage:`wesync-versioning(7)`,
:manpage:`wesync-faq(7)`
