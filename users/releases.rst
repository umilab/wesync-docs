...
Since the 1.0.0 release, Wesync uses a `semver
...
For releases obtained from Wesync.net or GitHub, with built-in upgrade
...
- ``wesync/wesync:latest`` points to the latest stable release, e.g. ``v1.9.0``
- ``wesync/wesync:rc`` points to the latest release candidate, e.g. ``v1.10.0-rc.3``
- ``wesync/wesync:nightly`` points to the latest nightly development build, e.g. ``v1.10.0-rc.3.dev.1.gd2e32957``

There are also tags for the major and minor versions, for example
``wesync/wesync:1.9`` for the latest stable 1.9 patch release and
``wesync/wesync:1`` for the latest stable release with major version
one.
...
- Change "Releases URL" to ``https://upgrades.wesync.net/nightly.json``
- Enable "Upgrade To Pre Releases"

Restart Wesync and it will upgrade to the latest nightly. New nightlies
...
are on the candidate channel, your Wesync will "upgrade" from
...
This table lists the historically released versions of Wesync, which Go
version they were built with, and which date they were released.

.. csv-table:: Wesync Releases
   :file: releases.csv
   :header-rows: 1
   :align: left
