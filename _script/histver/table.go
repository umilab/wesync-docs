...
func (r *tableRow) fromVersion(ver string) error {
	// wesync v1.23.1-rc.1 "Fermium Flea" (go1.19.5 darwin-arm64) teamcity@build.wesync.net 2023-01-12 03:30:17 UTC [stnoupgrade]
	exp := regexp.MustCompile(`wesync (v\d+\.\d+\.\d+).*(go\d+\.\d+(?:\.\d+)?).*(\d{4}-\d{2}-\d{2}) `)
