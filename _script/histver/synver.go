...
func getReleases(ctx context.Context) ([]*github.RepositoryRelease, error) {
	...
	for {
		rels, resp, err := client.Repositories.ListReleases(ctx, "umilab", "wesync", opts)
		if err != nil {
			return nil, err
		}
...
func getReleaseVersion(rel *github.RepositoryRelease) (*tableRow, error) {
	...
	find := fmt.Sprintf("wesync-%s-%s", goos, runtime.GOARCH)
...
func getReleaseVersionZip(bs []byte) (*tableRow, error) {
	...
	return nil, fmt.Errorf("no wesync binary found")
}
