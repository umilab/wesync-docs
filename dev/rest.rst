...
Wesync exposes a REST interface over HTTP on the GUI port. This is used by
the GUI (from Javascript) and can be used by other processes wishing to control
Wesync. In most cases both the input and output data is in JSON format. The
...
(add ``-k`` flag when using HTTPS with a Wesync generated or self signed
certificate).
