...
There's a lot that happens behind the covers, and Wesync is generally
...
Under Unix (including Mac) the easiest way to run Wesync with an
environment variable set is to prepend the variable to the command line.
I.e:

``$ STTRACE=model wesync``

On windows, it needs to be set prior to running Wesync.
...
::
    ...
    $ STNODEFAULTFOLDER=1 STNOUPGRADE=1  STNORESTART=1 dlv --listen=:2345 --headless=true --api-version=2 exec ./wesync -- --home=./_test_config --no-browser
